...
using WakfuMod.ModSystems; // Para FootballTeam y FootballSystem

namespace WakfuMod.Content.Projectiles // Reemplaza WakfuMod si es necesario
{
    public class TymadorKickProjectile : ModProjectile
    {
        ...
        private static Asset<Texture2D> _redKickTextureAsset; // NUEVO
        private static Asset<Texture2D> _blueKickTextureAsset; // NUEVO

        public override void Load()
        {
            if (!Main.dedServ) // Cargar solo en cliente
            {
                _kickTextureAsset = ModContent.Request<Texture2D>("WakfuMod/Content/Projectiles/TymadorKickProjectile"); // Ruta a tu sprite
                                                                                                                         // Asegúrate de que las rutas de patada roja/azul sean correctas
                _redKickTextureAsset = ModContent.Request<Texture2D>("WakfuMod/Content/Projectiles/RedKickProjectile"); // O el nombre que le diste
                _blueKickTextureAsset = ModContent.Request<Texture2D>("WakfuMod/Content/Projectiles/BlueKickProjectile"); // O el nombre que le diste
            }
        }
        public override void Unload()
        {
            _kickTextureAsset = null; // Descargar textura
            _redKickTextureAsset = null; // NUEVO
            _blueKickTextureAsset = null; // NUEVO
        }
        ...
        public override void SetDefaults()
        {
            ...
            Projectile.localNPCHitCooldown = 50; // Cooldown corto entre golpes al MISMO NPC (ajusta)
                ...
                if (bomb != null && bomb.active && bomb.owner == owner.whoAmI && !_hitBombsThisKick.Contains(bomb.whoAmI))
                {
                    if (kickHitbox.Intersects(bomb.Hitbox))
                    {
                        ...
                        if (bomb.ModProjectile is TymadorBomb tymadorBombInstance)
                        {
                            ...
                            float potentialNewBonus = tymadorBombInstance.kickDamageBonusPercent + bonusToAdd;
        ...
        private void CheckAndPushFriendlyProjectiles()
        {
            ...
            Vector2 strongKickVelocity = new Vector2(owner.direction * 11.2642767f, -15.2642767f);
        ...
        {
            Player owner = Main.player[Projectile.owner]; // Necesitamos al dueño

            // Determinar el equipo del jugador
            FootballTeam team = FootballTeam.None;
            if (owner.active && owner.TryGetModPlayer<WakfuPlayer>(out var wp))
            {
                team = wp.currentFootballTeam; // Obtener equipo desde WakfuPlayer
            }

            // Seleccionar la textura correcta
            Asset<Texture2D> selectedAsset = team switch
            {
                FootballTeam.Red => _redKickTextureAsset,
                FootballTeam.Blue => _blueKickTextureAsset,
                _ => _kickTextureAsset // Default si no está en equipo o hay error
            };

            // Verificar textura seleccionada
            if (selectedAsset == null || !selectedAsset.IsLoaded) { return false; }
            Texture2D texture = selectedAsset.Value; // Obtener la Texture2D

            // --- El resto del código de PreDraw es igual ---
            int frameHeight = texture.Height / AnimationFrames;
            int currentFrame = Projectile.frame % AnimationFrames;
            ...
            drawPosBase.Y += VisualVerticalOffset;

            Main.EntitySpriteDraw(
                texture, // <--- Usa la textura seleccionada
                drawPosBase,
                sourceRectangle,
                drawColor,
                rotation,
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            // --- Opcional: Anular Knockback ---
            // El knockback viene del item (6 o 12), si quieres anularlo:
